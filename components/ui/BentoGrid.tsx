...
export const BentoGrid = ({
  ...
  return (
    <div
      className={cn(
        "grid md:auto-rows-[9rem] grid-cols-1 md:grid-cols-6 gap-4 max-w-7xl mx-auto ",
        className
      )}
    >
      {children}
...
export const BentoGridItem = ({
  ...
  const handleCopy = () => {
    const text = "hajjejhazem063@gmail.com";
  ...
  return (
    <div
      className={cn(
        "row-span-1 relative overflow-hidden rounded-xl group/bento hover:shadow-xl transition duration-200 shadow-input dark:shadow-none p-4 dark:bg-black dark:border-white/[0.2] bg-white border-2 border-red-500 justify-between flex flex-col space-y-4 border-white/[0.1]",
        className
      )}
      style={{ background: 'rgb(4,7,29)', backgroundColor: 'linear-gradient(90deg, rgba(4,7,29,1) 0%, rgba(12,14,35,1) 100%', }}
    >
      <div className={`${id === 6 && 'flex justify-center'} ${id} relative h-full`}>
        ...
        <div className={cn(titleClassName, 'group-hover/bento:translate-x-2 transition duration-200 md:h-full min-h-40 flex flex-col px-5 p-5 lg:p-10')}>
